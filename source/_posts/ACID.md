...
### 脏读
脏读就是事务 A 读到了事务 B 未提交的数据，而这个数据后来被事务 B 回滚了。  
...
| T2 | 开始事务 | |
| T3 | | 查询余额为 1000 元 |
| T4 | | 存入 500 元，余额改为 1500 元 |
| T5 | 查询余额为 1500 元（脏读）| |
| T6 | | 回滚事务，余额恢复为 1000 元 |

### 不可重复读
不可重复读就是在事务 A 中，多次执行相同的查询却得到了不同的结果，这是因为在某次查询前后，事务 B 对符合事务 A 查询条件的数据进行了修改并提交。  
...
| T6 | | 提交事务 |
| T7 | 查询余额为 900 元（与第一次读不一致）| |

### 幻读
幻读就是在事务 A 中，多次执行相同的查询却得到了不同的结果，这个不同并不是列值不同，而是记录数不同。原因是在某次查询前后，事务 B **添加（删除不算）**了符合事务 A 查询条件的记录。这里主要参考 [ANSI SQL-92 规范中的定义](http://www.contrib.andrew.cmu.edu/~shadow/sql/sql1992.txt)。

| 时间点 | 事务 A | 事务 B |
| ------------ | ------------ | ------------ |
| T1 | | 开始事务 |
| T2 | 开始事务 | |
| T3 | 统计所有用户存款为 10000 元 | |
| T4 | | 新增一个用户，存款为 100 元 |
| T5 | | 提交事务 |
| T6 | 再次统计，存款为 10100 元 | |

### 丢失更新
丢失更新分为两类，第一类是事务 A 和事务 B 都修改数据，事务 A 先行修改后提交，之后事务 B 修改后回滚导致事务 A 提交的更新丢失；第二类是事务 A 和事务 B 都修改数据并先后提交，后提交的事务将先提交的事务的更新覆盖。  
...
| T3 | 查询余额为 1000 元 | |
| T4 | | 查询余额为 1000 元 |
| T5 | | 存入 100 元，余额改为 1100 元 |
| T6 | | 提交事务 |
| T7 | 取出 100 元，余额改为 900 元 | |
| T8 | 回滚事务 | |
